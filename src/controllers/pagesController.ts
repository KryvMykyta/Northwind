...
export class pagesController {
  ...
  constructor(pool: Pool) {
    this.repository = new PageRepository(pool);
  ...
  public getCustomers = async (
    ...
    try {
      ...
      const formattedData = this.formatter.addAvatarCustomer(
        rawCustomersData.data
      );

      const totalPagesFormat = await this.formatter.addTotalPages(
        rawCustomersData.sqlQueries,
        page,
        "customers",
        count
      );
  ...
  public getEmployees = async (
    ...
    try {
      ...
      const formattedData = this.formatter.addAvatarEmployee(
        rawCustomersData.data
      );
      const totalPagesFormat = await this.formatter.addTotalPages(
        rawCustomersData.sqlQueries,
  ...
  public getOrders = async (
    ...
    try {
      ...
      const formattedResponse = this.formatter.groupOrderPageResponse(
        ordersPage,
        firstIdQuery.sqlQueries[0]
      );

      const totalPagesData = await this.formatter.addTotalPages(
        formattedResponse.sqlQueries,
        page,
        "orders",
        count
      );
  ...
  public getSuppliers = async (
    ...
    try {
      ...
      const formattedData = this.formatter.addAvatarSupplier(
        rawCustomersData.data
      );
      const totalPagesFormat = await this.formatter.addTotalPages(
        rawCustomersData.sqlQueries,
